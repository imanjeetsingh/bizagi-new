...
function apiPageCall(page) {
    ...
    handleTiles('loading');
    fetch(api + page, {}).then(response => response.json())
        .then(data => {
            handleTiles(data);
...
function handlePagination(obj) {
    if(obj.pages && obj.pages > 1) {
        ...
        if (obj.currentPage < obj.pages - 2) {
            paginationhtml.push(`<li class="page-item"><a class="page-link disabled">...</a></li>`);
        }
        paginationhtml.push(`<li class="page-item ${obj.currentPage === obj.pages ? 'active' : ''}"><a class="page-link" href="#" data-page="${obj.pages}">${obj.pages}</a></li>`);
